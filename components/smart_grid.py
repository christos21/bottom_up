...
class SmartGrid:
    ...
    def set_generators(self, pv_df=None, csv_file=None, pv_from_array=False, pv_array=None):
        ...
        for pv in df.columns:
            pv_rated = df[pv]['rated']
            single_phase = True if df[pv]['phase_number'] in [1, 2, 3] else False

            pv_profile = 0 if ('PV_profile' not in df[pv].index or np.isnan(df[pv]['PV_profile'])) \
                else df[pv]['PV_profile']

            if single_phase:
                assert df[pv]['phase_number'] in [1, 2, 3]
                selected_phase = PHASES[df[pv]['phase_number'] - 1]
            else:
                selected_phase = None

            self.generators[pv] = PV(pv, pv_rated, single_phase, selected_phase, pv_profile)
